...
<section id="blog" class="my-10 py-10 mx-auto w-full max-w-screen-lg min-h-screen scroll-snap-start">
  ...
  <div class="flex gap-8">
    <div class="grid grid-cols-1 grid-rows-auto w-1/2 place-content-start">
      ...
      {#each posts as post}
      <article class="group py-10 pr-6 min-h-full" on:mouseover={updatePreview(post)}>
          <a 
            class="text-3xl leading-relaxed font-bold text-gray-900 group-hover:py-1 group-hover:bg-gray-100 transform duration-200"
            rel="prefetch" 
            href="/blog/{ post.slug }">
              { post.title }
          </a>
    ...
    <div class="w-1/2 col-span-1" style="">
        <img src="{previewImage}" alt="" class="rounded-md my-8 sticky top-20 shadow-lg">
    </div>
  </div>
</section>